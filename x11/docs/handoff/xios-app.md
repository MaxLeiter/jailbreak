...
5. **Session-switch jetsam resilience**: app-side teardown/re-adopt work exists; keep validating flavor switches under real memory pressure because two compositor surfaces can still produce transient peaks. The picker status line should show launcher progress (see session-launcher.md).
