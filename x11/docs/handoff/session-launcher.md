...
- `package-session.sh` → current local source package version is `xios-session_1.0.29_iphoneos-arm64.deb`, built under `linux-build/out/` and copied to top-level `repo/debs/`. `install-xios-session.sh` — lead-run scp+chmod. The package is target-aware for rootless/rootful staging; rootless installs cleanly after declaring the `xios-start-a11y` helper takeover from `iosc-shell <= 0.9.9`.
...
## Current state — package-installed 2026-07-04, CLI/daemon works
...
- KDE pickup: `run-kde-plasma.sh` is in the session manifest. It starts outer `iosc` with `IOSC_FRAME_PULSE=1`, launches `kwin_wayland` on `kwin-ios-test`, then starts `plasmashell` on that KWin socket under the same `dbus-run-session`. As of `xios-session 1.0.29`, the desktop preset defaults to the real upstream `org.kde.plasma.desktop` shell, and the old `org.kde.plasma.xios` first-light shell is no longer shipped by the session package. The launcher also keeps the KDE menu bridges rooted at `/var/jb/etc/xdg/menus` so KService sees shipped app entries. Device proof after first-light removal: `xios-session kde-desktop` reached status `up`, launched `plasmashell --shell-plugin org.kde.plasma.desktop`, did not reference `org.kde.plasma.xios`, and KWin imported Plasma IOSurfaces via EGL; evidence bundle `artifacts/device-runs/kde-desktop-no-firstlight-20260705-0029/`. Next deploy should pair this package with the current Xios direct-touch policy build, `plasma-workspace 6.1.5+ios8`, and `systemsettings 6.1.5+ios3`.
