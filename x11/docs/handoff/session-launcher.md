...
- `package-session.sh` → current local source package version is `xios-session_1.0.46_iphoneos-arm64.deb`, built under `linux-build/out/` and copied to top-level `repo/debs/`; SHA256 `f53dfad85904686dd753534ac5cf009f9c60e2fa7f5e5cd0585dfd9abbe7290c`. This package carries the KDE Qt platformtheme/Breeze env bridge, KDE color-scheme app-data bridge, Desktop fullscreen outer-KWin placement, guarded stale Desktop state migration, Mobile portrait sizing/wallpaper/Folio app-grid seeding, Mobile pre-foreground before KWin, Mobile `IOSC_FULLSCREEN_TOPLEVELS`/`IOSC_NO_OUTPUT_TRANSFORM` defaults, broader KDE app-bundle teardown, non-foregrounding slot sessions, and slot child cleanup. `install-xios-session.sh` — lead-run scp+chmod. The package is target-aware for rootless/rootful staging; rootless installs cleanly after declaring the `xios-start-a11y` helper takeover from `iosc-shell <= 0.9.9`.
