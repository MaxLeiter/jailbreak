...
- `package-session.sh` → current local source package version is `xios-session_1.0.39_iphoneos-arm64.deb`, built under `linux-build/out/` and copied to top-level `repo/debs/`. This package carries the KDE Qt platformtheme/Breeze env bridge, KDE color-scheme app-data bridge, Desktop raw-fit sizing, stale-favorites migration, and Mobile-only portrait auto-swap. `install-xios-session.sh` — lead-run scp+chmod. The package is target-aware for rootless/rootful staging; rootless installs cleanly after declaring the `xios-start-a11y` helper takeover from `iosc-shell <= 0.9.9`.
...
- KDE pickup: `run-kde-plasma.sh` is in the session manifest. It starts outer `iosc` with `IOSC_FRAME_PULSE=1`, launches `kwin_wayland` on `kwin-ios-test`, then starts `plasmashell` on that KWin socket under the same `dbus-run-session`. As of `xios-session 1.0.39`, the desktop preset defaults to the real upstream `org.kde.plasma.desktop` shell, exports the KDE Qt platformtheme/Breeze style when installed, seeds Breeze Dark defaults, bridges KPackage app-data paths including `color-schemes`, migrates stale Desktop favorites/configs, and keeps Desktop on `IOSC_LOGICAL=1440x1080`/KWin `1360x1000` so the panel is not clipped after iosc output rotation. Mobile keeps the portrait auto-swap path for its shell. The old `org.kde.plasma.xios` first-light shell is no longer shipped by the session package. Device proof after the UI style/layout pass: `xios-session kde-desktop` reached status `up`, launched `plasmashell --shell-plugin org.kde.plasma.desktop`, kept the real `desktop.so` KIO worker alive, and captured a transformed Desktop panel plus Xios wallpaper at `artifacts/device-runs/kde-ui-style-clean-layout-final-20260705-194801/`.
