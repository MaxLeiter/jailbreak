...
- **KDE/KF6 has KWin first-light, QtWayland/ANGLE IOSurface smoke, `xios-session kde`/`kde-desktop` launching the real upstream Plasma Desktop shell, a live KIO `desktop:` worker, a stable detached KWin/plasmashell launcher path, and a published/smoked first app batch (`ark`, `gwenview`, `kwrite`).** Next gate is replacing invalid default Desktop favorites, adding the missing session backend, and replacing remaining Mobile/Nano first-light shims with real service bridges.
