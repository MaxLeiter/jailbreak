...
- **KDE/KF6 has KWin first-light, QtWayland/ANGLE IOSurface smoke, packaged `plasma-workspace`/`plasmashell`, and a host-prepped `xios-session kde` launcher.** Next gate is `plasma-desktop` containment/applets plus on-device launcher validation.
