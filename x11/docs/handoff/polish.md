...
- Design committed `x11/docs/a11y-plan.md`; native-flavor half `x11/docs/native-ipados-a11y.md` has the HostA11y publisher prototype wired to send `bind{appid,exec}` and a smoke-only `/var/jb/tmp/xios-a11y-force` enable gate. Wire schema is the authoritative NDJSON schema in that doc. Shipped Linux-side P0/P1 smoke slices: `xios-session_1.0.9`/ioscd launch paths support opt-in `XIOS_ENABLE_A11Y=1` or force-file a11y, start `at-spi-bus-launcher`, write `IsEnabled` + `ScreenReaderEnabled` true, auto-start `xios-a11yd` when installed, avoid duplicate daemon starts, and kill the daemon on session switch; `xios-a11y-tools_0.2.7` installs `atspi-dump` and the snapshot daemon with `/var/jb/tmp/xios-a11y.sock` owned `mobile:mobile` mode 0660 for UIKit hosts. On-device smoke saw `application name="kgx"` / frame `iosc-kgx` on the AT-SPI bus, then socket clients received `hello`/`reset`/`window`/`upsert`; `bind{appid:"org.gnome.Console",exec:"kgx"}` published kgx and a non-matching bind published no windows. Forced native Console smoke then had HostA11y connect, bind, attach scene 3, and publish 12 elements onto the host view. The helper still polls but suppresses unchanged reset/tree republishes and now routes `activate`/custom `action` requests to AT-SPI Action.DoAction for published node ids, with synthetic-tap fallback for activate-without-action; `adjust` now routes to AT-SPI Value.SetCurrentValue and snapshots publish AT-SPI value text/current values. Direct `0.2.6` probe saw kgx node `1003` expose `actions:["overview.open"]`; stable `0.2.7` Console smoke still published 12 nodes with no socket close but no value-bearing nodes. A fresh native Calculator smoke published 18 elements and one action-rich node, but labels were empty and roles were mostly panel/grouping, so full GTK widget semantics remain open. Real VoiceOver action/adjust gestures are still pending. Default remains `GTK_A11Y=none` unless `XIOS_ENABLE_A11Y=1` or the force file exists. Still open: real VoiceOver gesture validation, full GTK widget tree validation, Qt package/device validation, event/focus/action smoke, PID correlation, geometry correlation for desktop Xios, focus-event support, and GTK3/GNOME Shell ATK version-skew fix. Memory: x11-a11y-voiceover-bridge.
