...
- target descriptor lint
- no unclassified rootless literals in generated rootful packages
- `dpkg-deb -I` control fields match target
- `dpkg-deb -c` payload paths match target
- Mach-O `LC_BUILD_VERSION` floors are reflected in `MinimumOSVersion`
- dependency-closure floor JSON is reproducible
...
Implement Phase 1's target descriptor skeleton:

- add `linux-build/targets/rootless-1900.env`
- add `linux-build/targets/rootful-1900.env`
- add `linux-build/target-lib.sh`
- add a `linux-build/print-target.sh` smoke helper
- keep all existing build commands defaulting to rootless behavior
