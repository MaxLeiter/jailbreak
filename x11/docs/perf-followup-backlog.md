...
P0.1 ✅ VALIDATED on-device 2026-07-01 — GTK4 app (kgx) renders GSK-ngl → ANGLE Metal → 3 rotating IOSurfaces → zero-copy into iosc. The shim fix chain (iosc-protocols commits: extension injection for EGL_*_platform_wayland + eglGetPlatformDisplayEXT-via-eglGetProcAddress with EGLint attribs matching xios_egl.c exactly + WINDOW→PBUFFER config rewrite) is correct + necessary. KEY BLOCKER was NOT the shim: GTK apps need GPU IOKit entitlements (AGXDeviceUserClient + IOGPUDeviceUserClient) or MTLCreateSystemDefaultDevice returns nil → ANGLE eglGetPlatformDisplay returns NO_DISPLAY → GSK falls back to CPU. REMAINING (packaging, lead): re-sign GTK app binaries with GPU ents + flip IOSC_GSK_RENDERER=ngl default + deploy shim as standing libEGL. Original finding:
...
P0.4 No frame pacing: every commit triggers an immediate synchronous full recomposite; frame callbacks fire instantly.
...
P0.6 Client-side glFinish per swap in the EGL shim.
...
P1.5 xdg-activation token set_app_id is a silent no-op (iosc.c:1788-1790) — tokens don't carry app_id, so activation policy can't ever key on it. Minor today (single-user), one-liner to store.
...
P2.1 iosc.c megafile refactor is QUEUED and the file keeps growing: refactor-plan.md says 4021 lines; it's now 5733 (plus XScreen.swift 1676). Plan's precondition list (panel + input-unification landed, blend fix validated, freeze announced) — input-unification (P1.1) is still open. XScreen.swift split is low-contention and can go first per the plan.
...
P2.6 Doc staleness: iosc-desktop-env.md §7 still says iosc "cannot" raise by app_id and requests the feature — it HAS landed (wm control socket + wm_find_toplevel_by_app_id, iosc.c:4898-4940, socket up at iosc.c:5705). refactor-plan.md line counts stale (P2.1). Worth a doc pass so future agents don't re-implement.
...
P2.8 kgx quirk: needs `-e <cmd>` to launch (memory note) — root-cause the default-shell spawn rather than carrying the workaround into launcher .desktop files.
