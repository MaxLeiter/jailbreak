...
if [ -n "${XIOS_SESSION_SLOT:-}" ]; then
  ...
  XIOS_CLIPBOARD_SOCKET="${XIOS_CLIPBOARD_SOCKET:-$XS_TMP/mutter-$XIOS_SESSION_SLOT-clipboard.sock}"
else
  ...
  XIOS_CLIPBOARD_SOCKET="${XIOS_CLIPBOARD_SOCKET:-$XS_TMP/mutter-clipboard.sock}"
fi
...
rm -f "$WSOCK" "$WSOCK.lock" "$XIOS_DDX_SOCKET" "$XIOS_JSON_PATH" \
      "$XIOS_INPUT_SOCKET" "$XIOS_CLIPBOARD_SOCKET" "$GNOME_SHELL_LOG" \
