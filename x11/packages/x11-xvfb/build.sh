#!/usr/bin/env bash
# Assemble x11-xvfb for a selected Xios target from a target-built Xvfb binary.
set -euo pipefail
...
TMPL="$X11DIR/packages/templates/x11-xvfb"
...
XVFB_SRC="$OUT/targets/$XIOS_TARGET_ID/Xvfb"
if [ "$XIOS_TARGET_ID" = "rootless-1900" ] && [ ! -f "$XVFB_SRC" ]; then
  XVFB_SRC="$PKGDIR/var/jb/usr/bin/Xvfb"
fi
[ -f "$XVFB_SRC" ] || {
  echo "missing target-built Xvfb for $XIOS_TARGET_ID: $XVFB_SRC" >&2
...
STAGEROOT="$X11DIR/linux-build/stage/$XIOS_TARGET_ID/x11-xvfb"
...
cp "$XVFB_SRC" "$PAYLOAD_BIN/Xvfb"
chmod 0755 "$PAYLOAD_BIN/Xvfb"

echo "==> staged x11-xvfb for $XIOS_TARGET_ID at $STAGE"
