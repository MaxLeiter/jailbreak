...
| File | Role |
|---|---|
| `ioscpanel.c` | shared shell client for `ioscbar`, `ioscdock`, and legacy `ioscpanel`: status bar + Control Center, floating dock, launcher/running-window activation, and the old combined panel fallback. |
...
```sh
./build-panel.sh        # -> out/ioscbar, out/ioscdock, out/ioscpanel, out/ioscoverview, out/ioscbg
