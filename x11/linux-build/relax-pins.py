...
Dry-run by default; --apply repacks the affected .debs in place.
"""
import os, io, re, sys, tarfile, subprocess, tempfile, shutil
...
for fn in sorted(os.listdir(DEBS)):
    ...
    repo.setdefault(field(txt, "Package"), set()).add(field(txt, "Version"))
...
print(f"\n{'APPLIED' if apply else 'DRY-RUN'}: {touched} deb(s) with unsatisfiable exact pins")
