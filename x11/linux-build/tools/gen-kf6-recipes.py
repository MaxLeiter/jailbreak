...
TABLE = [
    ...
    dict(t="karchive", kind="kf", deb="kf6-archive", deps=[],
         ...
         flags=["-DCMAKE_DISABLE_FIND_PACKAGE_BZip2=TRUE",
                "-DCMAKE_DISABLE_FIND_PACKAGE_LibLZMA=TRUE"],
         desc="Reading, creating and manipulating archive formats (zip, tar, 7z).",
         notes=["bzip2/xz backends disabled on purpose: zlib comes from the SDK, but the",
    ...
    dict(t="kguiaddons", kind="kf", deb="kf6-guiaddons", deps=[],
         ...
         notes=["APPLE-branch risk (audit): we build with CMAKE_SYSTEM_NAME=Darwin, and KDE",
    ...
    dict(t="kidletime", kind="kf", deb="kf6-idletime", deps=[],
         ...
         notes=["APPLE-branch risk: the mac idle poller uses ApplicationServices APIs that do",
    ...
    dict(t="solid", kind="kf", deb="kf6-solid", deps=[],
         ...
         notes=["Audit: on APPLE the backend is IOKit (the Linux path wants UDev). The IOKit",
                "backend is written against the macOS IOKit framework and may not compile on",
                "the iPhoneOS SDK; acceptable bring-up fallback is patching the backend list",
                "down to the fake/empty backend (KIO only needs the lib to link).",
    ...
    dict(t="knotifications", kind="kf", deb="kf6-notifications", deps=["kconfig"],
         ...
         notes=["Two audited patches, applied in -setup: (1) Canberra demoted from REQUIRED;",
