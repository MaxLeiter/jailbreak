...
# KIO's DBus-backed ForwardingWorkerBase/KDirNotify sources are needed by
# Plasma Workspace's real desktop:/ worker, so let iOS use the Unix QtDBus
# branch instead of the upstream APPLE exclusion. The NTFS hidden-attribute
# helper already has Darwin getxattr() arguments, but the guard is
# Q_OS_MACOS-only upstream; extend it to Q_OS_IOS. The trash worker's macOS
# DiskArbitration device-id branch is not available on the iPhoneOS SDK; keep
# the worker but use the generic Solid/stat path.
