...
xkeyboard-config: xkeyboard-config-setup xorgproto libx11 libxcb libxau libpthread-stubs libxdmcp
	...
	# x11-on-iOS override: stock recipe uses bare $(call AFTER_BUILD), which stages
	# for packaging but does NOT copy into build_base. gnome-desktop's meson does
	# dependency('xkeyboard-config') and needs xkeyboard-config.pc visible in the
	# sysroot, so copy the staged tree into build_base like every other build-dep.
